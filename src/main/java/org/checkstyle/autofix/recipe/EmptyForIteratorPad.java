...
public class EmptyForIteratorPad extends Recipe {
    ...
    private final class EmptyForIteratorPadVisitor extends JavaIsoVisitor<ExecutionContext> {
        ...
        private boolean isViolationDetected(J.ForLoop forLoop) {
            ...
            if (!hasViolation) {
                ...
                if (updateList.size() == 1
                        && updateList.get(0).getElement() instanceof J.Empty) {
                    if (hasViolationMarker(updateList.get(0).getElement())) {
                        hasViolation = true;
                    }
                }
            }
        ...
        private J.ForLoop applyFix(J.ForLoop forLoop) {
            ...
            if (updateList.size() == 1 && updateList.get(0).getElement() instanceof J.Empty) {
                final JRightPadded<Statement> emptyUpdate = updateList.get(0);
                final J.Empty emptyElement = (J.Empty) emptyUpdate.getElement();
        ...
        private boolean hasViolationMarker(J tree) {
            ...
            if (tree == null) {
                ...
                result = tree.getMarkers().findAll(CheckstyleViolationMarker.class).stream()
                        .anyMatch(marker -> marker.isFor(CheckFullName.EMPTY_FOR_ITERATOR_PAD));
            }
            return result;
        }
